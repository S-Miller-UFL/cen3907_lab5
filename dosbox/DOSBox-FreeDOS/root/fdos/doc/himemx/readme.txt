  ...
  UMBM is assumed to be located behind UMBPCI in CONFIG.SYS, and before the
  XMS driver. This will allow to load the XMS driver (and the EMM) into an
  UMB, thus saving some conventional DOS memory. After DOS has grabbed the
  UMBs, UMBM will remove itself from DOS memory.
